...
while DE
    ...
    % bulid SVR model
    k = oodacefit(A,B);
    MdlStd = fitrsvm(A,B,'Standardize',true, 'KernelFunction','rbf');
    for m=1:NP
        [~, temp_mse] = k.predict(x(m,:));
        temp_objF = MdlStd.predict(x(m,:));
        fv(m) = temp_objF-ww*sqrt(temp_mse);        %LCB
    end
    ...
    for gen = 1:G
        ...
        % selection option
        for m=1:NP
            [~, temp_mse] = k.predict(u(m,:));
            temp_objF = MdlStd.predict(u(m,:));
            fvu(m) = temp_objF-ww*sqrt(temp_mse);        %LCB
            if fvu(m) < fv(m)
                x(m, :) = u(m, :);
                fv(m) = fvu(m);
            end
